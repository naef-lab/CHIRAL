...
J.tilde<-function(E,n.genes=0,n.samples=0){
  ...
  #J<-cov(E) #Felix: let's keep in mind that this is normalized by N
  sda<-as.matrix(E)
...
Zeta.mf.ordered<-function(J, beta, samples, A.0=0.1){
  iterations<-1000
  time_symmetry<-0
  #FELIX: is the starting value for A important?
  A<-rep(A.0, samples)
  ...
  for (time in 1: iterations) {
    ...
    for (k in 1: samples) {
      ...
      # v<-0
      # for(i in 1:samples){
      #   u<-u+beta*A[i]*J[k,i]*cos(Theta[i])
      #   v<-v+beta*A[i]*J[k,i]*sin(Theta[i])
      # }
      u=beta*sum(A.c*J[k,])
      v=beta*sum(A.s*J[k,])
  ...
  #Theta <-Theta%%(2*pi)
  # FELIX: try this to break symmetry
...
Phase_reconstruction<- function(E, iterations=500, clockgenes=NULL,tau2=NULL, u=NULL, sigma2=NULL, TSM=TRUE, mean.centre.E=TRUE, q=0.1, update.q=FALSE, wt=0, pbar=TRUE, pca=FALSE, pca_comp=30, normalize_small=FALSE,blood=FALSE, phi.start=NULL, rem.1.svd=FALSE, supervised.start=FALSE, standardize=FALSE, multi_tix=FALSE){
  require(MASS)
...
Fit_OUT<-function(OUT,period=24){
  for(i in names(OUT)){ 
    E=OUT[[i]]$E
    E[abs(E)>5]=NA
    phase=OUT[[i]]$phi
    ...
    dat.fit=as.data.frame(t(apply(E, 1, harm_reg, 12*as.numeric(phase)/pi, period)))
    #dat.fit=subset(dat.fit, pval<1.5)
    dat.fit$qval=p.adjust(dat.fit[,1], "BH")
    OUT[[i]]$data.fit=data.frame(dat.fit,E,genes=rownames(E))
    colnames(OUT[[i]]$data.fit)=c("pval","phase", "amp", "a","b", "period", "R2","qval", colnames(E), "genes")
    if(is.null(OUT[[i]]$type)){OUT[[i]]$type="GTEx"}
...
Plot_cSVD<-function(input, genes, full_col=NULL,loc=NULL, ENSG=F, ncomp=NULL, CT=15, ymax=1.5, dot_size=1.5, label_size=3.5, text_size=12, sectors=1){
  if(!is.null(full_col)){
  full_col=full_col[full_col$Class!="Cells",]
  dec_names=full_col$`Short name`
  names(dec_names)=full_col$`Full name`
  ...
  for(i in 1:ncomp){
    ...
    print(qplot(Arg(SVD$u[,i])%%(2*pi)*12/pi,Mod(SVD$u[,i]))+geom_point(size=dot_size/2)+
            geom_label_repel(aes(label=labs),size = label_size)+coord_polar()+ylim(0,ymax)+ theme_minimal()+
    ...
    print(qplot(Arg(SVD$v[,i])%%(2*pi)*12/pi,Mod(SVD$v[,i]),colour=colz, shape=shepz)+geom_point(size=dot_size/2)+ 
            ...
            geom_label_repel(aes(label=deco_names[rownames(SVD$v)]),size = label_size)+theme(legend.position = "none")+
