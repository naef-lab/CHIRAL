#General analysis for MF separated
E.matrix=list()
load("data/female.RData")
load("data/male.RData")
for(name in names(CPM.all.norm)){
  ...
  if(!is.null(E$E) && any(gsub("\\|.*$","",gsub("^.*_", "",rownames(E$E)))=="PER3")){
    ...
    if(length(idx)>48){
      ...
      ml$phi=E$phi[gsub("\\..*$","", gsub("GTEX.","", colnames(ml$E)))]
      ml$type="GTEX-male"
      ml$tissue=name
      if(ncol(ml$E)>=24){E.matrix[[paste(name, "Male", sep="-")]]=ml}
      fm$E=E$E[,colnames(E$E) %in% female]
      fm$phi=E$phi[gsub("\\..*$","", gsub("GTEX.","", colnames(fm$E)))]
      fm$type="GTEX-female"
      fm$tissue=name
      if(ncol(fm$E)>=24){E.matrix[[paste(name, "Female", sep="-")]]=fm}
    }
  }
}
OUT=E.matrix
...
ggplot(alpha_mid)+geom_boxplot(aes(x=reorder(genes, -R2, FUN = mean),y=R2,fill=div))+theme(axis.text.x = element_text(angle = ang, hjust=hjst),text = element_text(size=sz))+scale_fill_manual(values=c("green3","blue","red","darkred"))
ggplot(alpha_mid)+geom_boxplot(aes(x=reorder(genes, -W, FUN = mean),y=W,fill=div))+theme(axis.text.x = element_text(angle = ang, hjust=hjst),text = element_text(size=sz))+scale_fill_manual(values=c("green3","blue","red","darkred"))
ggplot(alpha_mid)+geom_boxplot(aes(x=reorder(genes, -R, FUN = mean),y=R,fill=div))+theme(axis.text.x = element_text(angle = ang, hjust=hjst),text = element_text(size=sz))+scale_fill_manual(values=c("green3","blue","red","darkred"))
